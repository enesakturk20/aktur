      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-28">
          {services.map((service, index) => (
            <div
              key={service.id}              className={`relative group overflow-hidden rounded-lg shadow-lg cursor-pointer h-64 md:h-80 ${
                services.length % 2 !== 0 && index === services.length - 1 ? 'md:col-span-2 md:w-1/2 md:mx-auto' : ''
              }`}
            >
              {/* Resim */}
              <img
